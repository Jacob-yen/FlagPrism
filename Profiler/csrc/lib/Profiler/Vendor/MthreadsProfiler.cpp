...
struct MthreadsMuptiApi {
  using RegisterCallbacks = MUptiResult (*)(
      MUpti_BuffersCallbackRequestFunc, MUpti_BuffersCallbackCompleteFunc);
  ...
  bool load() {
    ...
    if (const char *env = std::getenv(
            "FLAGTREE_PROFILER_MTHREADS_MUPTI_LIBRARY")) {
      if (*env) {
    ...
    candidates.emplace_back("/usr/local/musa-4.3.5/lib/libmupti.so");
    ...
    activityEnable = reinterpret_cast<ActivityEnable>(
        dlsym(library, "muptiActivityEnable"));
    ...
    unsubscribe = reinterpret_cast<Unsubscribe>(
        dlsym(library, "muptiUnsubscribe"));
    enableDomain = reinterpret_cast<EnableDomain>(
        dlsym(library, "muptiEnableDomain"));
    return registerCallbacks && activityEnable && activityDisable &&
           activityFlushAll && activityGetNextRecord;
  }
};
...
collectMthreadsFiles(const SessionProfileMetadata &metadata) {
  ...
  for (const auto *key : {"mupti_import_path", "mthreads_import_path",
                          "vendor_import_path", "output_path",
                          "mupti_output_path", "mthreads_output_path"}) {
  ...
  if (const char *env =
          std::getenv("FLAGTREE_PROFILER_MTHREADS_IMPORT_PATH")) {
    if (*env) {
...
void importCsv(const std::filesystem::path &file,
  ...
  const auto endIndex = findColumn(
      headers, {"end_time_ns", "end_time_us", "end_time", "end"});
  ...
  while (std::getline(input, line)) {
    ...
    association.runtimeEvent.deviceId = parseU64(cell(row, deviceIndex)).value_or(0);
    association.runtimeEvent.streamId = parseU64(cell(row, streamIndex)).value_or(0);
    association.runtimeEvent.taskId = parseU64(cell(row, taskIndex)).value_or(0);
    ...
    for (size_t i = 0; i < runtimeEvents.size(); ++i) {
      if (!usedRuntimeEvents[i] &&
          sameKernel(association.runtimeEvent.opName,
...
void MthreadsProfiler::consumeMuptiBuffer(uint8_t *buffer, size_t validSize) {
  ...
  while (true) {
    const auto result =
        api.activityGetNextRecord(buffer, validSize, &record);
    ...
    const auto *kernel = reinterpret_cast<const MUpti_ActivityKernel6 *>(
        record);
    NativeKernelEvent event;
...
bool MthreadsProfiler::startMuptiCapture() {
  ...
  if (!subscriberEnabled && api.subscribe && api.enableDomain) {
    MUpti_SubscriberHandle subscriber = nullptr;
    const auto result = api.subscribe(
        &subscriber,
...
void MthreadsProfiler::writeMuptiOutput() {
  ...
  if (events.empty()) {
    std::lock_guard<std::mutex> lock(mutex);
    nativeCaptureError =
        std::string("MUPTI capture produced no completed kernel activity records ") +
        "(buffer_requests=" + std::to_string(nativeBufferRequests) +
        ", buffer_completions=" +
        std::to_string(nativeBufferCompletions) + ", activity_records=" +
        std::to_string(nativeActivityRecords) + ")";
    return;
  }
  ...
  for (const auto &event : events) {
    ...
    output << ',' << event.startTimeNs << ',' << event.endTimeNs << ','
           ...
           << ',' << event.dynamicSharedMemory << ','
           << event.localMemoryPerThread << ',' << event.localMemoryTotal
           << ',';
...
void MthreadsProfiler::doFlush() { flushMuptiCapture(false); }
...
void MthreadsProfiler::doSetMode(
  ...
  for (const auto &raw : modeAndOptions) {
    ...
    } else if (key == "mupti_output_path" ||
               key == "mthreads_output_path" || key == "output_path") {
  ...
  if (nativeCaptureRequested && nativeOutputPath.empty()) {
    ...
    nativeOutputPath =
        (directory / ("flagtree_mthreads_mupti_" +
...
VendorProfileArtifact MthreadsProfiler::importMthreadsOutput(
  ...
  for (const auto &file : files) {
    ...
    const auto source = !nativeOutputPath.empty() &&
  ...
  if (files.empty() && !plan.enabledVendorMetrics.empty()) {
    artifact.degradeReasons.push_back(
        "No MUPTI activity export was found; runtime-base host timing was retained.");
  }
  return artifact;
}
