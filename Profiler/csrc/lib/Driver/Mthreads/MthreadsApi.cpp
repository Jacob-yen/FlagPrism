...
struct Api {
  ...
  bool load() {
    ...
    const char *env = std::getenv("FLAGTREE_PROFILER_MTHREADS_DRIVER_LIBRARY");
    ...
    if (const char *musaHome = std::getenv("MUSA_HOME")) {
      if (*musaHome) {
        candidates.emplace_back(std::string(musaHome) + "/lib/libmusa.so");
        candidates.emplace_back(std::string(musaHome) + "/lib64/libmusa.so");
      }
    }
    ...
    deviceGetName =
        reinterpret_cast<FnDeviceGetName>(dlsym(library, "muDeviceGetName"));
    deviceGetAttribute = reinterpret_cast<FnDeviceGetAttribute>(
        dlsym(library, "muDeviceGetAttribute"));
    if (init != nullptr && deviceGet != nullptr) {
      return true;
    }
    dlclose(library);
    library = nullptr;
